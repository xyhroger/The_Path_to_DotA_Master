...
function calculateIndex(leftHero, rightHero) {
    ...
    var theta = 2.0 / (1 + Math.exp((lane[1] - lane[0]) / 17.5));
    ...
    for (var i = 0; i < 2; i++){
        ...
        for (var j = 0; j < 7; j++) {
            ...
            team.push(theta*attr/math.mean(para)/para.length);
        }
        var srAlly = [];
        var srEnemy = [];
...
function winRateCal(teams) {
    var weight = [0.5, 0.5, 0.5, 0.5, 0.0, 0.0, 0.0, 6, 25.817];
...
function compare() {
    $(document).ready()
    {
        ...
        indexResult = [{
            ...
            {
                attribute: "ADVANCE-INDEXING",
                left: teams[0][7],
                right: teams[1][7]
            }];
        //alert(indexResult);
        var result = indexResult;
        var win = winRateCal(teams);
        // alert(win[0].toFixed(2)*100);
        var winLeft = parseFloat(win[0] * 100).toFixed(1);
        var winRight = parseFloat(win[1]* 100).toFixed(1)

        $(".win-rate-left").empty().append("<p class='percent-text'>" + winLeft + "%</p>");
        addOneBar(win[0].toFixed(2)*100, ".win-rate-left");
        $(".win-rate-right").empty().append("<p class='percent-text'>" + winRight + "%</p>");
        addOneBar(win[0].toFixed(2)*100, ".win-rate-right");

        drawCompareChart2(result);
    }
}
